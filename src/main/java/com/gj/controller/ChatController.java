...
public class ChatController {
    ...
    public ResponseMessage uploadChatImage(@PathVariable Integer userId, @RequestParam("file") MultipartFile file) {
        // 动态获取项目根目录
        ...
        chatService.uploadChatImage(userId, avatarPath); // 更新数据库
        return ResponseMessage.uploadsuccess("上传成功", baseUrl + avatarPath);

    }

    @PatchMapping("/clearUnread/{userId}/{friendId}")
    public ResponseMessage clearUnread(@PathVariable Integer userId, @PathVariable Integer friendId) {
       List<Chat> chatNew = chatService.clearUnread(userId, friendId);
        return ResponseMessage.success("已清空未读消息");
    }
